...
if (!foundUser.professional) return res.status(404).json({ message: "User not professional." });

// da controllare
const foundSmm = await Smm.find({ smmId: foundUser.username }).exec();

if (!foundSmm) return res.status(404).json({ message: "No VIP associated." });
